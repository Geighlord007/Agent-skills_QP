...
├── progress.md                # current phase, completed steps, pending user items, blockers
...
**`progress.md` checkpoints.** `progress.md` is the anti-interruption record. Update it after every major step and every phase transition, not just at the end of Phase A. Required update points are marked with **(↻ progress)** below. The file must always show:
- Current workflow phase and step
- Completed steps
- Pending user-confirmation items
- Known blockers

### Phase A — Research

1. **Scoping (Step 0).** Read `references/thread-library.md` and select threads appropriate to the topic and mode. Confirm scope with the user in manual mode; proceed in auto mode. **(↻ progress)**
2. **Thread research (Step 1).** Launch one subagent per thread using `AgentSwarm`. Each thread writes to `drafts/T{NN}_{topic}.md` and appends sources to `datas/sources.jsonl`. **(↻ progress)**
3. **Conflict resolution (Step 2).** Read all thread drafts, identify factual conflicts, and spawn focused verification subagents. Mark unresolved conflicts with `⚠️`. **(↻ progress)**
4. **Phase A wrap-up (Step 2.5).** Update `progress.md`, summarize 3-5 key findings and unresolved conflicts, and **wait for user approval before Phase B** (manual mode only). **(↻ progress)**

### Phase B — Report assembly

5. **Outline (Step 3).** Load `references/report-template.md`, map threads to sections, and produce a section outline saved to `drafts/outline.md`. **(↻ progress)**
6. **Section writing (Step 4).** Write one section at a time, reading only the relevant thread files and `datas/sources.jsonl`. Save `report.md` after every section. **(↻ progress after each section)**
7. **Source merge (Step 4.5).** Run `scripts/merge_sources.py` to normalize and deduplicate `datas/sources.jsonl`. **(↻ progress)**
8. **Citation verification (Step 5).** Run `scripts/verify_citations.py` to ensure every `[N]` in `report.md` resolves to a source and every source has a URL/PMID/patent number. **(↻ progress)**
9. **Validation gate (Step 6).** Run `scripts/validate_report.py`. If it fails, return to Step 4. **(↻ progress)**
10. **Quality evaluation (Step 7).** Apply `references/rubric.md` and save `eval/quality_evaluation.md`. **(↻ progress)**
11. **Fix & deliver (Step 8).** Fix only rubric-identified issues, re-run affected checks, convert to DOCX with `scripts/md_to_docx.py`, and deliver. **(↻ progress)**

### Interruptions, rollbacks, and comments

- If the user interrupts the task, stop immediately and update `progress.md` with the current step, reason for interruption, and any next-step hints.
- If the user asks to roll back, restore the appropriate `drafts/` version and record the reason in `progress.md`.
- If the user provides comments during modification mode, append them to `cmts.md`, process them, and update `progress.md` only when the change affects core claims, scope, or current blockers.
